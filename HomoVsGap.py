...
for r, a, N_magnets in param_sets:
    ...
    size_mm = int(a * 1000)
    r_mm = int(r * 1000)
    config_dir = f"Config_a{size_mm}_r{r_mm}"
    plots_dir = f"Plots_a{size_mm}_r{r_mm}"
    field_dir = f"field_a{size_mm}_r{r_mm}"
    ...
    for num_rings, gap_mm in tqdm(configs, desc=f"Simulating r={r}, a={a}, N={N_magnets}"):
        gap = gap_mm / 1000
        ...
        html_file = f"{config_dir}/Halbach_{num_rings}rings_gap{gap_mm}.html"
        if not os.path.exists(html_file):
            fig = magpy.show(halbach, backend='plotly', return_fig=True)
            fig.write_html(html_file)

        
        csv_file = f"{field_dir}/Halbach_{num_rings}rings_gap{gap_mm}_DSV.csv"
    ...
    summary_df.to_csv(
        f"{field_dir}/Halbach_summary.csv",
        index=False,
        float_format="%.6e"
    )
    all_results[(r, a, N_magnets)] = summary_df


# Plots
plt.figure()
for (r, a, N), df in all_results.items():
    plt.plot(df['Gap_mm'], df['Mean_By'], '-o',
             label=f'r={r}m, a={a*1000:.0f}mm, N={N}')
plt.xlabel('Gap [mm]')
...
for (r, a, N), df in all_results.items():
    plt.plot(df['Gap_mm'], df['Mean_absBx'], '-o',
             label=f'|Bx| r={r}m, a={a*1000:.0f}mm, N={N}')
...
for (r, a, N), df in all_results.items():
    plt.plot(df['Gap_mm'], df['Mean_absBz'], '-o',
             label=f'|Bz| r={r}m, a={a*1000:.0f}mm, N={N}')
